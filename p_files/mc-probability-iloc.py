...
def get_B_matrix(A, Q, S, r_max, p_succ):
        ...
        for f in erange(1, S):
            for r in erange(2, r_max):
                for t in erange(0, Q - 1):
                    ...
                    df.iloc[states.index((q, f, r, t)), states.index((q - 1, f - 1, r_max, t + 1))] = p_succ
                    # print(df)
                    # exit()
                    df.iloc[states.index((q, f, r, t)), states.index((q, f - 1, r - 1, t))] = p_fail
            # for t in erange(0, Q - 1):
            for t in erange(0, Q - 1): # todo Should this extra loop be here?
                df.iloc[states.index((q, f, 1, t)), states.index((q - 1, f - 1, r_max, t + 1))] = p_succ
                df.iloc[states.index((q, f, 1, t)), states.index((q - 1, f - 1, r_max, t))] = p_fail
    #
    # for f in erange(0, S):
    #     for r in erange(1, r_max):
    #         for t in erange(0, Q-1):
    ...
    for f in erange(1, S):
        for r in erange(1, r_max):
            for t in erange(0, Q-1):
                # print("(q = 0, f = {f}, r = {r}, t = {t}) -> (q = 0, f = {f2}, r = {r2}, t = {t2}) = 1.0".format(f=f, r=r, t=t, f2=f, r2=r, t2=t))
                df.iloc[states.index((0, f, r, t)), states.index((0, f, r, t))] = 1.0
    ...
    for q in erange(0, Q):
        for r in erange(1, r_max):
            for t in erange(0, Q-1):
                ...
                df.iloc[states.index((q, 0, r, t)), states.index((q, 0, r, t))] = 1.0
