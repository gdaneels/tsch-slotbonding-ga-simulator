...
for trx_payload_size in trx_payload_sizes:
    ...
    csv_list = fnmatch.filter(os.listdir(results_dir), "TX_%dB_AT*.csv" % trx_payload_size)
    tx_raw = pd.DataFrame()
    ...
    for index,phy in enumerate(phy_list):
        ...
        sns.regplot(x="RSSI",y="PRR",data=tx_raw[tx_raw["PHY"] == phy],order=5,ci=None,color=colors[index])
