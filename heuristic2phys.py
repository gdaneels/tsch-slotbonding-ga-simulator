...
class HeuristicDifferentPHYs:
    def __init__(self, nr_slots, phy_to_nr_frequencies, slots_per_MCS, settings_file, output_dir):
        ...
        self.PHY_F_MAX = dict()
        self.PHY_F = dict()
        for phy, nr_freq in phy_to_nr_frequencies.items():
            self.PHY_F_MAX[phy] = nr_freq - 1
            self.PHY_F[phy] = range(0, self.PHY_F_MAX[phy] + 1)

        # frequencies at which a node can transmit
        # self.F_MAX = phy_to_nr_frequencies['TSCH_SLOTBONDING_50_KBPS_PHY'] - 1  # maximal frequency offset
    ...
    def set(self, links, visualize_solution=False):
        ...
        for phy, f_max in self.PHY_F_MAX.items():
            self.schedule_tx[phy] = dict()
            self.schedule_rx[phy] = dict()
            self.write_schedule_tx = dict()
            for n in self.N:
                if n != self.settings['topology']['root']:
                    self.write_schedule_tx[n] = {}
                self.schedule_tx[phy][n] = []
                self.schedule_rx[phy][n] = []
                for freq in range(0, f_max + 1):
                    self.schedule_tx[phy][n].append([False for ts in range(0, self.T_MAX + 1)])
                    self.schedule_rx[phy][n].append([False for ts in range(0, self.T_MAX + 1)])
    ...
    def valid_tx_rx_n(self, node, ts):
        # we check for both PHYs
        # for TX, we should only check the PHY of the node
        # for RX, we should check both PHY schedules, because you can receive on either PHY
        # for convenience of programming (we can put it in a loop), we check both PHYs for TX and RX
        for phy, f_max in self.PHY_F_MAX.items():
            for ch in range(0, f_max + 1):
                # if the node is transmitting or receiving at that place
                # check for the PHY = self.MCS[node] of the node
                if self.schedule_tx[phy][node][ch][ts] or self.schedule_rx[phy][node][ch][ts]:
    ...
    def valid_interferers(self, node, ts, ch, debug=False):
        ...
        # check the PHY = self.MCS[node] of the 'node', if the interferer is TX'ing in the same phy, it will be True
        for interferer in self.my_interferers[self.P[node]]:
            if self.schedule_tx[self.MCS[node]][interferer][ch][ts]:
                return False
        ...
        # here you only have to check the PHY of the node TX'ing, because the node in range can be RX'ing, it just can't be in the same PHY band
        for node_in_range in self.in_interference_range[node]:
            if self.schedule_rx[self.MCS[node]][node_in_range][ch][ts]:
                return False
        return True

    def valid_tx_rx_parent(self, node, ts):
        # we check for both PHYs
        # for TX, we should only check the PHY of the parent
        # for RX, we should check both PHY schedules, because you can receive on either PHY
        # for convenience of programming (we can put it in a loop), we check both PHYs for TX and RX
        for phy, f_max in self.PHY_F_MAX.items():
            for ch in range(0, f_max + 1):
                # the parent is already listening to the node's siblings or transmitting to its parents, return false
                if self.schedule_tx[phy][self.P[node]][ch][ts] or self.schedule_rx[phy][self.P[node]][ch][ts]:
                    return False
        return True
    ...
    def allocate_bonded_slot(self, node, ts, ch, length_bonded_slot, solution_file=False):
        ...
        while ts_min <= ts_max:
            self.schedule_tx[self.MCS[node]][node][ch][ts_min] = True
            self.schedule_rx[self.MCS[node]][self.P[node]][ch][ts_min] = True
            ts_min += 1
        if solution_file:
    ...
    def check(self, visualize_solution=False, solution_file=False):
        ...
        for n, depth_tuple, nr_slots, nr_contenders, length_tuple in self.sorted_N_0:
            ...
            if bonded_slots_to_allocate > 0:
                ...
                for ch in range(0, self.PHY_F_MAX[self.MCS[n]] + 1):
        ...
        if feasible:
            for phy, phy_f in self.PHY_F.items():
                if visualize_solution or solution_file:
                    tmp_schedule = dict()
                    viz = Visualization(len(self.T), len(phy_f), self.N, self.P, interferers=self.my_interferers)
                    for n in self.N_0:
                        for t in self.T:
                            for f in phy_f:
                                if self.schedule_tx[phy][n][f][t]:
                                    viz.add_node(t, f, n, self.Y[n])
                    # viz.visualize(suffix='ga-{0}'.format(phy), output_dir=self.settings["ga"]["results_dir"])
                    viz.visualize(suffix='ga-{0}'.format(phy), output_dir=self.output_dir)
